...
class PredictionViewModel constructor(private val useCase: EaseplantzUseCase) : ViewModel() {
    fun getPrediction(model: String, image: MultipartBody.Part) : LiveData<Resource<Prediction>>{
        return LiveDataReactiveStreams.fromPublisher(useCase.getPrediction(model, image))
    }
}